-- Migração 2026-09-22
-- Objetivo: gold_counts — mapa jsonb { generatorId → count }
-- Raridade gold (preferencial vs shiny/negativo): shiny + gold ≤ qty.
--
...
ALTER TABLE despertar_states
  ADD COLUMN IF NOT EXISTS gold_counts jsonb NOT NULL DEFAULT '{}'::jsonb;
...
BEGIN
  ...
  SET
    ...
    END,
    gold_counts = CASE
      WHEN v_patch ? 'gold_counts' THEN COALESCE(v_patch->'gold_counts', '{}'::jsonb)
      ELSE ds.gold_counts
    END,
    upgrades_state = CASE
...
COMMENT ON FUNCTION public.despertar_persist_and_award(integer, jsonb, text[], integer) IS
  'Fase B / B3 + G4.3 + gold: persiste patch (incl. shiny_counts, gold_counts) + append de conquistas/xp. Sem validação de economia.';
